...
class Reversi:
    def __init__(self):
        pyxel.init(SCREEN_WIDTH, SCREEN_HEIGHT, title="Pyxel Reversi")
        pyxel.mouse(True)
        self.board = Board()
        self.current = "black"
    ...
    def draw(self):
        ...
        if (
            ...
            and self.board.can_move(ROWS[r_idx], COLS[c_idx], self.current)
        ):
            pyxel.rect(
                SQUARE_SIZE * r_idx, SQUARE_SIZE * c_idx, SQUARE_SIZE, SQUARE_SIZE, 9
            )
        ...
        pyxel.rect(
            PASS_BTN_X, PASS_BTN_Y, PASS_BTN_W, PASS_BTN_H, 9 if can_pass else 13
        )
        ...
        pyxel.text(
            4,
            SCREEN_WIDTH + 5,
            "Black's turn" if self.current == "black" else "White's turn",
            7,
        )
        # ゲーム終了メッセージの描画
        if self.winner:
